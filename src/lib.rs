...
impl App {
    ...
    pub async fn fetch_and_process_emails(&self) -> Result<()> {
        ...
        for (i, email) in emails.iter().enumerate() {
            if let Some(transaction) = email::parse_transaction_from_email(email) {
                ...
                if i < 5 {
                    // Only show first 5 non-parsed emails for debugging
                    ...
                    println!(
                        "[Email {}] Could not parse transaction from email (preview):\n{}...",
                        i + 1,
                        preview.replace("\n", " ")
                    );
                }
    ...
    pub async fn list_transactions(
        ...
        let transactions = self
            .db
            .get_transactions(r#type.as_deref(), from.as_deref(), to.as_deref(), limit)
            .await?;
