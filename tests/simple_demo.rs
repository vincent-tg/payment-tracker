...
use payment_tracker::{App, config::Config, db, email};
use std::fs;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tokio::runtime::Runtime::new()?.block_on(async {
        ...
        // Income
        app.add_manual_transaction(1000.00, "Salary", "in", Some("2024-01-15"))
            .await?;
        println!("   Added: Salary - $1000.00 (in)");

        // Expenses
        app.add_manual_transaction(50.00, "Groceries", "out", Some("2024-01-16"))
            .await?;
        println!("   Added: Groceries - $50.00 (out)");

        app.add_manual_transaction(25.00, "Coffee", "out", Some("2024-01-17"))
            .await?;
        println!("   Added: Coffee - $25.00 (out)");

        app.add_manual_transaction(100.00, "Electricity Bill", "out", Some("2024-01-18"))
            .await?;
        println!("   Added: Electricity Bill - $100.00 (out)");

        app.add_manual_transaction(200.00, "Freelance Work", "in", Some("2024-01-19"))
            .await?;
