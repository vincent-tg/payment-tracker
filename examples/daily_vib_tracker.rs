...
async fn main() -> anyhow::Result<()> {
    ...
    for (i, email_text) in emails.iter().enumerate() {
        ...
        if let Some(transaction) = payment_tracker::email::parse_transaction_from_email(email_text) {
            ...
            println!("  Email #{}: No transaction found (not a VIB email or different format)", i + 1);
        }
    }
    ...
    let recent_transactions = db.get_transactions(
        None,  // type filter
    ...
    // Calculate totals
    let total_in = recent_transactions.iter()
        .filter(|tx| tx.r#type == "in")
        .map(|tx| tx.to_usd())
        .sum::<f64>();
    
    let total_out = recent_transactions.iter()
